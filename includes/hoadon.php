...
class HoaDon
{
    ...
    public function themHoaDon($tenkh, $ngaytao, $totalAll, $trangthai, $productList)
    {
        $sql = "INSERT INTO hoadon (MaKH, NgayTao,TongTien,TrangThai) VALUES (?,?,?,?)";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("isis", $tenkh, $ngaytao, $totalAll, $trangthai);
        $result = $stmt->execute();
        if ($result) {
